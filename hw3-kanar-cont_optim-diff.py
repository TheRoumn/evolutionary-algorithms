...
CR_PROB = 0.9 # probability of flipping each bit in mutation
...
EXP_ID = 'differential_complete_random_k3' # the ID of this experiment (used to create log names)
...
def differential_selection(pop, fits, k, mutate_ind, cross_ind, fit):
    ...
    for i in range(k):
        if fits[i] > candidate_fits[i].fitness:
...
def evolutionary_algorithm(pop, max_gen, fitness, operators, mate_sel, cross_ind, mutate_ind, *, map_fn=map, log=None):
    ...
    for G in range(max_gen):
        ...
        # exp_decay_step = exp_decay(G, 0, max_gen, 1, 0.5)
        make_donors = functools.partial(get_donor, pop, k=1, f=f)
